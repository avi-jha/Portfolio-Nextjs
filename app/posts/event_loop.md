...
The **event loop** is one of the most notorious topics in JavaScript.  
It often sounds complicated, but once you understand how JavaScript executes code, it becomes surprisingly straightforward.
...
The **call stack** manages the execution of your JavaScript code.  
JavaScript is single-threaded, which means it can execute only one piece of code at a time.


Each line of code (or function call) is pushed onto the call stack and executed in order.  
Once execution finishes, it’s popped off the stack.
...
These APIs are **not part of JavaScript itself**—they’re provided by the browser environment.  
They can be **callback-based** or **promise-based**.
